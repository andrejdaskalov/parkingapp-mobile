  ...
  Widget build(BuildContext context) {
    return BlocProvider(
      create: (context) {
        ...
        bloc.add(GetParkingDetails());
        return bloc;
      },
      child: BlocListener<PaymentBloc, PaymentState>(
        listener: (context, state) {
          if (state.status == ParkingStatus.stopped) {
            // if (context.canPop()) {
            //   context.pop();
            // } else {
            //   context.go('/');
            // }
            context.goNamed("contribute", pathParameters: {"contribId": state.currentlyPayingParking!});
          }
        },
        child: BlocBuilder<PaymentBloc, PaymentState>(
          builder: (context, state) {
            ...
            } else if (state.currentlyPayingParking == null || state.status == ParkingStatus.stopped) {
              return Padding(
                  padding: MediaQuery
                      .of(context)
                      .padding +
                ...
                body: Container(
                  ...
                  child: Column(
                    ...
                    children: [
                      ...
                      SubtitleHeading(text: "Почеток: ${state.startTime?.hour
                          .toString().padLeft(2, '0')}:${state.startTime?.minute
